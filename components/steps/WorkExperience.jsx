...
export default function WorkExperience() {
    ...
    const [currentSkillInputs, setCurrentSkillInputs] = useState({})
    ...
    const addSkill = (expIndex, skillText) => {
        ...
        const updatedExperiences = experiences.map((exp, i) => {
            if (i === expIndex) {
                ...
                return {
                    ...exp,
                    skills: [...exp.skills, skillText.trim()]
                }
            }
            return exp
        })
        setExperiences(updatedExperiences)
        setCurrentSkillInputs(prev => ({
            ...prev,
            [expIndex]: ""
        }))
    }
    ...
    const handleSkillKeyDown = (expIndex, e) => {
        if (e.key === 'Tab' || e.key === 'Enter') {
            e.preventDefault()
            const skillText = currentSkillInputs[expIndex] || ""
            if (skillText.trim()) {
                addSkill(expIndex, skillText)
            }
        }
    }
                            ...
                            <div key={index} className="space-y-6 relative">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        className={cn("w-full min-h-[100px] border-gray-300 resize-none", boldGrayFocus)}
                                    />
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div
                                            className={cn(
                                                "flex flex-wrap items-center w-full min-h-10 rounded-md border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm transition-colors",
                                        ...
                                        >
                                            ...
                                            {exp.skills.map((skill, skillIndex) => (
                                                <div
                                                    key={skillIndex}
                                                    className="flex items-center gap-1 bg-gray-200 text-gray-800 px-2 py-0.5 rounded-full text-xs mr-2 my-0.5"
                                                >
                                                    <span>{skill}</span>
                                                    <button
                                                        type="button"
                                            ...
                                            {/* Skill Input */}
                                            <input
                                                placeholder={exp.skills.length >= 5 ? "" : "Type a skill and press Tab"}
                                                value={currentSkillInputs[index] || ""}
                                                onChange={(e) => handleSkillInputChange(index, e.target.value)}
                                                onKeyDown={(e) => handleSkillKeyDown(index, e)}
                                                disabled={exp.skills.length >= 5}
                                                className="flex-1 bg-transparent border-none outline-none ring-0 focus-visible:ring-0 shadow-none h-auto p-0 text-sm disabled:cursor-not-allowed disabled:opacity-50 min-w-[50px] my-0.5"
                                            />
                                        </div>

                                        {exp.skills.length >= 5 && (
                                            <p className="text-xs text-red-500">Maximum 5 skills allowed</p>
                                        )}
                                    </div>
                                </div>
                            </div>
